...
class BidService {
  async placeBid(projectId, amount, period, description, token = null, bidderId = null) {
      ...
      try {
      const selected = useUsersStore.getState().getSelectedUser?.();
      if (!bidderId && selected?.user_bid_id) {
        bidderId = Number(selected.user_bid_id);
        console.log(`Using bidderId from sub-user store: ${bidderId}`);
      }
      if (!token && selected?.sub_user_access_token) {
        token = selected.sub_user_access_token;
        console.log(`Using token from sub-user store for user: ${selected.sub_username || selected.id}`);
      }
    } catch (zErr) {
      console.warn('Could not read selected sub-user from store:', zErr);
    }
