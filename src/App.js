...
//       <Route path="/ai-templates/add" element={<SimpleAddTemplateForm />} />
...
const ProtectedRoute = ({ children, requireAdmin = false }) => {
  ...
  if (requireAdmin && fbUser.role !== 'admin') return <Navigate to="/" replace />;
  return children;
};

function App() {
  // read current auth loading state (keeps same gating as before)
  const { user: fbUser, loading: fbLoading } = useFirebaseAuth();
            ...
            <NotificationProvider>
              <ModalProvider>
                <Routes>
                  ...
                  <Route path="/subusers" element={<ProtectedRoute><SubUserRegister parentUid={fbUser?.uid} /></ProtectedRoute>} />

                  {/* App shell: MainLayout keeps Navbar/Sidebar; nested pages render inside it */}
                  <Route path="/" element={<ProtectedRoute><MainLayout /></ProtectedRoute>}>
                    <Route index element={<Dashboard />} />

                    {/* More settings and filter tabs keep MainLayout (and Sidebar if configured) */}
                    <Route path="more-settings/*" element={<MoreSettings />}>
                     <Route index element={<FilterContainer activeTab="update-profile" />} />
                      <Route path=":tab" element={<FilterContainer />} />
                    </Route> />

                    {/* AI templates — keep inside app shell so layout doesn't change */}
                    <Route path="ai-templates" element={<AITemplatesPage />} />
                    <Route path="ai-templates/add" element={<SimpleAddTemplateForm />} />

                    {/* other app routes can be added here as nested children */}
                  </Route>

                  {/* legacy redirects (optional) */}
                  <Route path="/ai-templates/*" element={<Navigate to="/ai-templates" replace />} />
